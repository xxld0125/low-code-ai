...
import type { Notification, NotificationAction } from '@/lib/designer/conflict-detection'
...
export function NotificationSystem({
  ...
  useEffect(() => {
    if (isOpen) {
      notifications.filter(n => !n.read && !n.persistent).forEach(n => onMarkAsRead(n.id))
    }
...
export function ToastNotification({
  ...
  return (
    <div
      className={cn(
        'fixed right-4 top-4 z-50 max-w-sm rounded-lg border p-4 shadow-lg transition-all duration-300',
        getNotificationClasses(notification.type),
