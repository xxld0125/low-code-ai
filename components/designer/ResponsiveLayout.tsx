...
export function ResponsiveLayout({
  ...
  const getMainContentStyles = () => {
    ...
    const sidebarWidth = sidebarCollapsed
      ? sidebarBehavior.collapsedWidth
      : sidebarBehavior.expandedWidth
    return {
  ...
  return (
    ...
    >
      ...
      {isMobile && rightPanelOpen && (
        <div className="fixed inset-0 z-40 bg-black/50" onClick={() => setRightPanelOpen(false)} />
      )}
      ...
      {/* Main Content */}
      <main className="flex min-w-0 flex-1 flex-col" style={getMainContentStyles()}>
        {/* Header */}
        {header && (
          <header className="flex items-center justify-between border-b border-border bg-card px-4 py-3">
            {/* Mobile menu button */}
            {isMobile && sidebar && (
              <button onClick={toggleSidebar} className="mr-2 rounded-md p-2 hover:bg-muted">
            ...
            {isMobile && rightPanel && (
              <button onClick={toggleRightPanel} className="ml-2 rounded-md p-2 hover:bg-muted">
                <Menu className="h-4 w-4" />
              </button>
            )}
          </header>
