...
export async function acquireTableLock(
  ...
  try {
    ...
    if (lockCheckError) {
      throw new LockError(
        'Failed to check existing locks',
        'DATABASE_ERROR',
    ...
    if (activeLock) {
      ...
      throw new LockError(
        `Table is already locked by ${lockOwner}`,
        'ALREADY_LOCKED',
        {
    ...
    throw new LockError(
      'Unexpected error occurred while acquiring lock',
      'UNEXPECTED_ERROR',
      { originalError: error }
    )
  }
...
export async function releaseTableLock(
  ...
  try {
    ...
    if (lock.user_id !== user.id) {
      throw new LockError(
        'You can only release your own locks',
        'FORBIDDEN'
      )
    }
    ...
    throw new LockError(
      'Unexpected error occurred while releasing lock',
      'UNEXPECTED_ERROR',
      { originalError: error }
    )
  }
 ...
 */
export async function getTableLocks(
  projectId: string,
  tableId: string
): Promise<TableLock[]> {
  const supabase = createClient()
  ...
  try {
    ...
    throw new LockError(
      'Unexpected error occurred while fetching locks',
      'UNEXPECTED_ERROR',
      { originalError: error }
    )
  }
...
export async function cleanupExpiredLocks(): Promise<number> {
  ...
  try {
    ...
    throw new LockError(
      'Unexpected error occurred during cleanup',
      'UNEXPECTED_ERROR',
      { originalError: error }
    )
  }
...
export async function extendLock(
  ...
  try {
    ...
    if (lock.user_id !== user.id) {
      throw new LockError(
        'You can only extend your own locks',
        'FORBIDDEN'
      )
    }
    ...
    throw new LockError(
      'Unexpected error occurred while extending lock',
      'UNEXPECTED_ERROR',
      { originalError: error }
    )
  }
