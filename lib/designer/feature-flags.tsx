  ...
  return (
    <div className="fixed bottom-4 right-4 z-50">
      <button
        ...
        className="rounded-lg bg-black/80 p-2 text-xs text-white"
      >
        🚩 Flags ({Object.values(flags).filter(Boolean).length})
      </button>

      {isVisible && (
        <div className="absolute bottom-10 right-0 max-h-96 w-96 overflow-y-auto rounded-lg bg-black/90 p-4 text-xs text-white">
          <h3 className="mb-2 font-bold">Feature Flags</h3>

          {error && (
            <div className="mb-2 rounded border border-red-500 bg-red-500/20 p-2">
              Error: {error}
            </div>
          )}
