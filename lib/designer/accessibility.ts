...
export function useKeyboardNavigation(
  ...
  const { orientation = 'vertical', loop = true, onSelectionChange, onAction } = options
...
export function checkWCAGCompliance(
  ...
  const aaThreshold = isLargeText
    ? COLOR_CONTRAST_RATIOS.WCAG_AA_LARGE
    : COLOR_CONTRAST_RATIOS.WCAG_AA_NORMAL
  const aaaThreshold = isLargeText
    ? COLOR_CONTRAST_RATIOS.WCAG_AAA_LARGE
    : COLOR_CONTRAST_RATIOS.WCAG_AAA_NORMAL
...
export function useScreenReader() {
  ...
  useEffect(() => {
    ...
    const checkScreenReader = () => {
      ...
      const hasVoiceOver =
        /Mac|iPhone|iPad/.test(navigator.userAgent) && navigator.userAgent.includes('VoiceOver')
      ...
      const isLikelyScreenReader =
        hasNvda || hasJaws || hasVoiceOver || (hasMotionPreference && hasContrastPreference)
